...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const rows = await executeQuery('SELECT id, name, slug, subtitle, description, icon, is_hidden, created_at, updated_at FROM specializations WHERE id = ?', [id]);
    ...
    if (req.method === 'PUT' || req.method === 'PATCH') {
      const { name, description, icon, is_hidden, subtitle } = req.body || {};
    ...
    if (req.method === 'POST') {
      ...
      const updated = await executeQuery('SELECT id, name, slug, subtitle, description, icon, is_hidden, created_at, updated_at FROM specializations WHERE id = ?', [id]);
