...
export default function RadiologyPage({ service }: PageProps) {
              ...
              <div className="mt-10 sm:mt-14">
                {service.heroImage ? (
                  ...
                  <div className="w-full h-56 sm:h-[360px] md:h-[480px] bg-neutral-400 rounded-md flex items-center justify-center text-white text-sm sm:text-base font-medium select-none">
                    Media / Ilustrasi Radiologi (Belum Tersedia)
                  </div>
                )}
              </div>
            </div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    service = { slug, title: 'Layanan Radiologi', description: 'Konten belum tersedia.', sections: [] };
  }
  return {
    props: { service },
    revalidate: 60
